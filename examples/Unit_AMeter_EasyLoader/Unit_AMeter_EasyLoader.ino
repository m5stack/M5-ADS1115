...
// Define meter size as 1 for M5.Lcd.rotation(0) or 1.3333 for
// M5.Lcd.rotation(1)
...
void loop() {
    if (updateTime <= millis()) {
        ...
        value[0] = current;
        plotNeedle(value[0], 0);  // It takes between 2 and 12ms to replot the
                                  // needle with zero delay
    }
}
...
void analogMeter() {
        ...
        if (i % 25 == 0) {
            ...
            switch (i / 25) {
                case -2:
                    ...
                    break;
                case -1:
                    M5.Lcd.drawCentreString("500", x0, y0 - 9, 2);
                    break;
                case 0:
                    M5.Lcd.drawCentreString("1000", x0, y0 - 7, 2);
                    break;
                case 1:
                    M5.Lcd.drawCentreString("1500", x0, y0 - 9, 2);
                    break;
                case 2:
                    M5.Lcd.drawCentreString("2000", x0, y0 - 12, 2);
                    break;
            }
        }
    ...
    M5.Lcd.drawString("mA", M_SIZE * (5 + 230 - 40), M_SIZE * (119 - 20),
                      2);  // Units at bottom right
    M5.Lcd.drawCentreString("mA", M_SIZE * 120, M_SIZE * 70,
                            4);  // Comment out to avoid font 4
    M5.Lcd.drawRect(5, 3, M_SIZE * 230, M_SIZE * 119,
                    TFT_BLACK);  // Draw bezel line
...
void plotNeedle(int value, byte ms_delay) {
    ...
    while (!(value == old_analog)) {
        ...
        float sdeg =
            map(old_analog, -200, 2200, -150, -30);  // Map value to angle
        ...
        M5.Lcd.drawLine(M_SIZE * (120 + 20 * ltx - 1), M_SIZE * (140 - 20),
                        osx - 1, osy, TFT_WHITE);
        M5.Lcd.drawLine(M_SIZE * (120 + 20 * ltx), M_SIZE * (140 - 20), osx,
                        osy, TFT_WHITE);
        //    M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx + 1), M_SIZE*(140 - 20),
        //    osx + 1, osy, TFT_WHITE);
        ...
        M5.Lcd.drawLine(M_SIZE * (120 + 20 * ltx), M_SIZE * (140 - 20), osx,
                        osy, TFT_MAGENTA);
        //   M5.Lcd.drawLine(M_SIZE*(120 + 20 * ltx + 1), M_SIZE*(140 - 20), osx
        //   + 1, osy, TFT_RED);
